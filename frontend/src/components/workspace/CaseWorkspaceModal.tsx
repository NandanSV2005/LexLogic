      ...
      <Card className="max-w-3xl w-full max-h-[90vh] flex flex-col p-6 space-y-4 overflow-hidden">
        ...
        {isLoading && (
          <div className="py-16 text-center space-y-3">
            <RefreshCw className="w-8 h-8 text-[#7C9A82] animate-spin mx-auto" />
            <p className="text-xs text-[#617066] font-semibold">Loading case workspace & timeline...</p>
          </div>
        )}
        ...
        {!isLoading && workspace && (
          <div className="flex-1 overflow-y-auto space-y-5 pr-1">
            {/* TABS HEADER */}
            <div className="flex items-center gap-2 border-b border-[#C8D7C7] pb-2">
              <button
                type="button"
                onClick={() => setActiveTab('overview')}
                className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${
                  activeTab === 'overview'
                    ? 'bg-[#DDE8DC] text-[#29352D] shadow-sm'
                    : 'text-[#617066] hover:bg-[#FAFCF9]'
                }`}
              >
            ...
            {activeTab === 'overview' && (
              <div className="space-y-4">
                ...
                <div className="p-4 bg-[#DDE8DC] border border-[#C8D7C7] rounded-2xl space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] font-bold text-[#7C9A82] uppercase tracking-wider flex items-center gap-1">
                      <Sparkles className="w-3.5 h-3.5" /> Next Recommended Action
                    </span>
                    <Badge variant="purple">{workspace.next_action.actor_role}</Badge>
                  </div>
                  <h3 className="text-sm font-extrabold text-[#29352D]">
                ...
                {workspace.connected_provider ? (
                  <Card className="p-4 space-y-2.5 bg-[#FAFCF9]">
                    <div className="flex items-center justify-between">
                      <span className="text-[10px] font-bold text-[#617066] uppercase tracking-wider flex items-center gap-1">
                        <UserCheck className="w-3.5 h-3.5 text-[#7C9A82]" /> Assigned Representation
                      </span>
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        <h4 className="text-sm font-extrabold text-[#29352D]">
                      ...
                      <div className="text-right text-xs">
                        ...
                        <span className="text-[10px] text-[#617066] block">Reliability</span>
                      </div>
                    </div>
                  </Card>
                ) : (
                  <Card className="p-4 text-center bg-[#FAFCF9]">
                    <Clock className="w-6 h-6 text-[#617066] mx-auto mb-2" />
                    <span className="text-xs font-bold text-[#29352D] block">No Assigned Representation Yet</span>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2">
                  {onOpenPrivacyCenter && (
                    <Button
                      ...
                      leftIcon={<Lock className="w-4 h-4 text-[#7C9A82]" />}
                  ...
                  {onOpenScheduling && (
                    <Button
                      ...
                      leftIcon={<Calendar className="w-4 h-4 text-[#7C9A82]" />}
                      className="w-full justify-start"
                    >
                ...
                <div className="p-4 bg-[#FAFCF9] border border-[#C8D7C7] rounded-xl space-y-1.5">
                  <span className="text-[10px] font-bold text-[#617066] uppercase tracking-wider block">
                    Legal Need Description
                  </span>
                  <p className="text-xs text-[#29352D] leading-relaxed whitespace-pre-wrap">
            ...
            {activeTab === 'timeline' && (
              <div className="space-y-3 relative before:absolute before:inset-0 before:left-3.5 before:w-0.5 before:bg-[#C8D7C7]">
                {workspace.timeline.map((evt) => (
                  <div key={evt.id} className="relative pl-8 space-y-1">
                    <div className="absolute left-1 top-1 w-5 h-5 rounded-full bg-[#7C9A82] text-white flex items-center justify-center text-[10px] font-bold shadow-sm">
                      ✓
                    </div>
                    <div className="p-3 bg-[#FAFCF9] border border-[#C8D7C7] rounded-xl text-xs space-y-1">
                      <div className="flex items-center justify-between">
                        <span className="font-extrabold text-[#29352D]">{evt.title}</span>
                        <span className="text-[10px] text-[#617066]">
                          {new Date(evt.timestamp).toLocaleString()}
                        </span>
                      </div>
                      <p className="text-xs text-[#617066] leading-relaxed">{evt.description}</p>
                    </div>
                  </div>
                ))}
              </div>
